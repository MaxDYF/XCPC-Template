 ...
 * Info 表示值，Tag 表示标记
 */
template <typename _Info, typename _Tag> class LCT {
  public:
    LCT() : m_size(0) {}
    LCT(const int &_n, const _Info &_v = _Info()) { init(_n, _v); }
    LCT(const std::vector<_Info> &_init) { init(_init); }
    void init(const int &_n, const _Info &_v = _Info()) { init(std::vector<_Info>(_n, _v)); }
  ...
  private:
    ...
    std::vector<_Tag> tag;
    size_t m_size;
    bool relation(const int &x) const { return ch[fa[x]][0] == x || ch[fa[x]][1] == x; }
...
struct Info {
    ...
    void apply(const Tag &v) { sum = (sum * v.mul % mod + v.add * siz % mod) % mod; }
    Info operator+(const Info &b) const { return Info(sum + b.sum, siz + b.siz); }
};
LCT<Info, Tag> tr;