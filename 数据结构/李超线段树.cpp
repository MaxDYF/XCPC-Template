...
private:
	...
	void _update(Line g, int p, int l, int r){
		...
		Line& f = tr[p].f;
		int mid = (l + r) >> 1;
		if(less(f, g, mid)) std::swap(f, g);
		if(l == r) return;
		if(less(f, g, l)) _update(g, p << 1, l, mid);
		if(less(f, g, r)) _update(g, p << 1 | 1, mid + 1, r);
	}
	void _insert(const Line& g, int x, int y, int p, int l, int r){
		tr[p].vis = true;
		if(x <= l && r <= y) return _update(g, p, l, r);
		int mid = (l + r) >> 1;
		if(x <= mid) _insert(g, x, y, p << 1, l, mid);
		if(y > mid) _insert(g, x, y, p << 1 | 1, mid + 1, r);
	}
	i64 _getmax(int x, int p, int l, int r)const{
		if(!tr[p].vis) return -inf;
		i64 ans = tr[p].has_line? tr[p].f.at(x): -inf;
		if(l == r) return ans;
		int mid = (l + r) >> 1;
		if(x <= mid) return std::max(ans, _getmax(x, p << 1, l, mid));
		else return std::max(ans, _getmax(x, p << 1 | 1, mid + 1, r));
		return ans;
	}
};