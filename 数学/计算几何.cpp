...
struct Vector {
    design_float x, y;
    Vector(design_float x = 0, design_float y = 0) : x(x), y(y) {}
...
struct Line {
    ...
    // 判断点c在线段上
    bool isPointOnSegment(Point c) { return fabs((b - a) ^ (c - a)) <= eps && (c - a) * (c - b) <= eps; }
    // 求点c到直线的距离
    design_float distanceToLine(Point c) { return fabs(((b - a) ^ (c - a)) / (b - a).length()); }
    // 求点c到线段的距离
...
namespace Geometry {
Point rotate(Point p, Point base, design_float angle) { return (p - base).rotate(angle) + base; }
...
bool pointOnLeft(Point p, Line l) { return ((l.b - l.a) ^ (p - l.a)) > eps; }
